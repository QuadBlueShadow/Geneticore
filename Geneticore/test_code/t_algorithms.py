...
class CRGeneticAlg:
  def __init__(self,  base_net=None, episodes_per_gen=1, num_nets=5, t_episodes=0, max_fitness=-10000):
    ...
    self.adj_fill = self.base_net.get_adj_n()
  ...
  def make_nets(self):
    ...
    for i in range(self.num):
      if i < self.num/10:
        new_net = copy.deepcopy(self.abs_best)
        new_net.randomly_adjust_net(self.adj_fill)
...
class BGeneticAlg:
  ...
  def make_nets(self):
    ...
    for i in range(self.num):
      if i < self.num/10:
        new_net = copy.deepcopy(self.abs_best)
        new_net.randomly_adjust_net(self.best_net.get_adj_n())
        self.nets.append(new_net)
      else:
        new_net = copy.deepcopy(self.best_net)
        new_net.randomly_adjust_net(self.best_net.get_adj_n())
